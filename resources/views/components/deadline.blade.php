<div class="container-xl mt-4">
    @if($tasks->isEmpty())
        <h5>日付指定タスク一覧</h5>
            <div class="pt-2">
                <div class="alert alert-success" role="alert">現在、日付設定されているタスクはありません。</div>
            </div>
    @endif
    ...
    <table class="table mt-2">
        ...
        <tbody>
            @foreach($tasks->groupBy('deadline') as $deadline => $taskGroup)
                <tr>
                    <th>
                    @if($taskGroup->isNotEmpty())
                        <span class="badge text-bg-success">{{ $deadline }}</span>
                    @endif
                    </th>
                </tr>
                ...
                <tr class="task-item">
                    <td class="d-flex align-items-center justify-content-between"id="task-{{ $item->id }}">
                            ...
                            <div class="flex-column d-flex">
                                <form method="post" action="{{ route('sort_task', $item->id) }}">
                                    ...
                                    <button class="btn btn-light btn-sm" type="submit">
                                        ...
                                        <input type="hidden" name="deadline" value="{{ request()->get('deadline') }}">
                                    </button>
                                </form>
                                <form method="post" action="{{ route('sort_task', $item->id) }}">
                                    @csrf
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            @if($item->taskList && $item->taskList->list != null)
                            <div class="my-box task-controls" data-task-id="{{ $item->id }}">
                                <span class="badge rounded-pill text-bg-info" id="list-{{ $item->id }}">{{ $item->taskList->list}}</span>
                            </div>
                            @endif
